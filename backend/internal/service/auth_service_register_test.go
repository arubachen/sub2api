...
func newAuthService(repo *userRepoStub, settings map[string]string, emailCache EmailCache, quotaRepo UserPlatformQuotaRepository) *AuthService {
	return newAuthServiceWithPromo(repo, settings, emailCache, nil, quotaRepo)
}

func newAuthServiceWithPromo(repo *userRepoStub, settings map[string]string, emailCache EmailCache, promoService *PromoService, quotaRepo UserPlatformQuotaRepository) *AuthService {
	cfg := &config.Config{
