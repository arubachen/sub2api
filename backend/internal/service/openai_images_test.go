...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_NormalizesPresentedOpenAIImageModel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"openai/gpt-image-2","prompt":"draw a cat","size":"1024x1024","response_format":"b64_json"}`)
...
func TestOpenAIGatewayServiceParseOpenAIImagesRequest_AcceptsPresentedGrokImagineImageModel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"x-ai/grok-imagine-image-lite","prompt":"draw a cat","size":"1024x1024","response_format":"b64_json"}`)
	...
	require.NoError(t, err)
	require.NotNil(t, parsed)
	require.Equal(t, "grok-imagine-image-lite", parsed.Model)
	require.Equal(t, OpenAIImagesCapabilityNative, parsed.RequiredCapability)
}
